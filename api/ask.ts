...
export default async function handler(request: Request): Promise<Response> {
  ...
  for (const message of messages) {
    ...
    // Tool results are generated by our own client and can legitimately be
    // longer than a typed question; user turns are held to the question cap.
    const cap = message.role === 'tool' ? 4_000 : MAX_QUESTION_CHARS;
    if (message.content.length > cap) {
      return json({ error: `message too long (max ${cap} characters)` }, 400);
    }
  }
